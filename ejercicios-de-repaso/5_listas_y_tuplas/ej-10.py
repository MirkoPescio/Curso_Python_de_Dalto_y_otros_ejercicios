"""
Ejercicio 10
Escribir un programa que almacene en una lista los siguientes precios,
50, 75, 46, 22, 80, 65, 8, y muestre por pantalla el menor y el mayor de los precios.
"""

# Mi solución:

lista_precios = [50, 75, 46, 22, 80, 65, 8]
lista_precios.sort()

precio_menor = lista_precios[0]
precio_mayor = lista_precios[-1]

print("El producto con menor precio cuesta $" + str(precio_menor))
print("El producto con mayor precio cuesta $" + str(precio_mayor))


# Solución de la guía:

prices = [50, 75, 46, 22, 80, 65, 8]
min = max = prices[0]
for price in prices:
    if price < min:
        min = price
    elif price > max:
        max = price 
print("El mínimo es " + str(min)) 
print("El máximo es " + str(max))

